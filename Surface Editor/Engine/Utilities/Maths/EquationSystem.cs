...
namespace Engine.Utilities
{
    ...
    {
        private readonly IIntersectable obj1;
        private readonly IIntersectable obj2;
        private readonly Func<float, float, Vector4> F3;
        private readonly bool _differentParams;

        public EquationSystem(IIntersectable obj1, IIntersectable obj2, Func<float, float, Vector4> f3, bool differentParams)
        {
            this.obj1 = obj1;
            this.obj2 = obj2;
            F3 = f3;
        ...
        public void Jacobian(float[] x, float[][] jacobian)
        {
            ...
            var f1Point1 = obj1.Evaluate(obj1.ClampU(x[0]), obj1.ClampV(x[1]));
            var f1Point2 = obj1.Evaluate(obj1.ClampU(x[0] + diff), obj1.ClampV(x[1]));
            var f1Point3 = obj1.Evaluate(obj1.ClampU(x[0]), obj1.ClampV(x[1] + diff));

            var f2Point1 = obj2.Evaluate(obj2.ClampU(x[2]), obj2.ClampV(x[3]));
            var f2Point2 = obj2.Evaluate(obj2.ClampU(x[2] + diff), obj2.ClampV(x[3]));
            var f2Point3 = obj2.Evaluate(obj2.ClampU(x[2]), obj2.ClampV(x[3] + diff));
        ...
        {
            var t1Point = obj1.Evaluate(obj1.ClampU(x[0]), obj1.ClampV(x[1]));
            var t2Point = obj2.Evaluate(obj2.ClampU(x[2]), obj2.ClampV(x[3]));
